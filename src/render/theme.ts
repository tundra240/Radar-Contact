...
export interface Palette {
  ...
  /**
   * How panel edges are drawn, which is the one thing about the interface
   * that a colour cannot express.
   *
   * `bevel` is the turn-of-the-century look: a light edge along the top and
   * left, a shadow edge along the bottom and right, swapped to read as
   * sunken. `flat` is what replaced it -- a single hairline border and a
   * change of fill, which is how every screen in a modern control room is
   * drawn. Both the canvas chrome and the stylesheet read this, so a scheme
   * cannot be half of one and half of the other.
   */
